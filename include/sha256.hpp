...
// Good enough for content-addressed build caching — not optimized
// for huge files, but markdown/template files are tiny.
...
private:
    ...
    void update(const uint8_t* data, size_t len) {
        ...
        size_t pos = 0;
        while (buffer.size() - pos >= 64) {
            processBlock(&buffer[pos]);
            pos += 64;
        }
    ...
    std::string hexDigest() {
        ...
        for (size_t pos = 0; pos < buffer.size(); pos += 64)
            processBlock(&buffer[pos]);
